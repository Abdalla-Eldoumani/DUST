...
export function RevealScreen({
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="fixed inset-0 z-50 flex items-center justify-center bg-void/90 p-4 backdrop-blur-sm md:p-6"
    >
      <motion.div
        initial={{ scale: 0.9, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        transition={{ type: "spring", damping: 20 }}
        className="flex w-full max-w-md flex-col border border-white/10 bg-surface max-h-[calc(100dvh-2rem)] overflow-hidden md:max-h-[calc(100dvh-3rem)]"
      >
        <div className="shrink-0 px-6 pt-6 pb-4">
          <h2 className="font-mono text-lg text-text-primary text-center uppercase tracking-wider">
            Archive Analysis
          </h2>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-4">
          ...
          {/* Score summary */}
          <div className="border-t border-white/5 pt-4 space-y-1 text-center">
